...
export default defineComponent({
    ...
    async setup() {
        ...
        const jsonImportFullTest = async (): Promise<void>  => {
            ...
            if(!res.result) return Promise.reject("failed in isJsonValid");
            ...
            if(res.changes.changes === -1 ) return Promise.reject("failed in importFromJson");
            setLog(log.value.concat("> importFromJson successful\n"));

            // create a connection for "db-from-json"
            res = await sqlite.createConnection("db-from-json", false, "no-encryption", 1);
            if(res == null ) return Promise.reject("failed in createConnection");
            if((Object.keys(res)).includes("result") && !res.result) return Promise.reject("failed in createConnection");
            setLog(log.value.concat("> createConnection " +
                                        " 'db-from-json' successful\n"));
            let db: SQLiteDBConnection = res; 
            // open db "db-from-json"
            res = await db.open();
            if (!res.result) return Promise.reject("failed in open");
            setLog(log.value.concat("> open " +
                                        " 'db-from-json' successful\n"));
            // create synchronization table 
            res = await db.createSyncTable();
            if (res.changes.changes < 0) return Promise.reject("failed in createSyncTable");
            setLog(log.value.concat("> createSyncTable " +
                                        " 'db-from-json' successful\n"));
            // get the synchronization date
            res = await db.getSyncDate();
            if(res.syncDate === 0) return Promise.reject("failed in getSyncDate");
            ...
            if(res.values.length !== 4 || 
                ...
                return Promise.reject("failed in query");
            }
            // close the connection
            res = await sqlite.closeConnection("db-from-json"); 
            if(!res.result) return Promise.reject("failed in closeConnection"); 
        ...
        const jsonImportPartialTest = async (): Promise<void>  => {
            ...
            if(res.values.length !== 6 || 
                ...
                return Promise.reject("failed in query users");
            }
            ...
            if(res.values.length !== 4|| 
                ...
                return Promise.reject("failed in query messages");
            }
            setLog(log.value.concat("> query " +
                                        " 'messages' successful\n"));
            ...
            if(res.values.length !== 2 || 
                ...
                return Promise.reject("failed in query images");
            }
            setLog(log.value.concat("> query " +
                                    " 'images' successful\n"));
        ...
        const jsonExportFullTest = async (): Promise<void>  => {
            ...
            // create a connection for "db-from-json"
            let res: any = await sqlite.createConnection("db-from-json", false, "no-encryption", 1);
            if(res == null ) return Promise.reject("failed in createConnection");
            if((Object.keys(res)).includes("result") && !res.result) return Promise.reject("failed in createConnection");
            setLog(log.value.concat("> createConnection " +
                                        " 'db-from-json' successful\n"));
            ...
            // test Json object validity
            res = await sqlite.isJsonValid(JSON.stringify(jsonObj.export));
            if(!res.result) {
                setLog(log.value.concat(`> isJsonValid ${res.message}\n`));
                return Promise.reject("failed in isJsonValid")
            }
        ...
        const jsonExportPartialTest = async (): Promise<void>  => {
            ...
            res = await db.setSyncDate("2020-05-20T18:40:00.000Z");
            if(!res.result) {
            return Promise.reject("failed in setSyncDate");
            }
            ...
            if(jsonObj.export.tables.length !== 3 || jsonObj.export.tables[0].name !== 'users'
                ...
                return Promise.reject("Json Object not correct");
            }
            ...
            if(!res) {
                setLog(log.value.concat("* deleteDatabase failed*\n"));
                return Promise.reject("failed in deleteDatabase");
            }      
